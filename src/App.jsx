...
export default function InvoiceApp() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setDbReady(true); // let user work offline
      }
    })();
  }, []);
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f0f2f5", fontFamily: "'Source Sans 3', 'Segoe UI', sans-serif" }}>
      {/* NAV */}
      <div style={{ background: "linear-gradient(135deg, #1a3a3a, #2a5a5a)", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between", boxShadow: "0 2px 12px rgba(0,0,0,0.15)" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
          <div style={{ width: 36, height: 36, background: "#2eb8b8", borderRadius: 8, display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, color: "white", fontSize: 16 }}>C</div>
          <div>
            <div style={{ color: "white", fontWeight: 700, fontSize: 16, letterSpacing: 1 }}>CODKAGE FACTURES</div>
            <div style={{ color: "#7cc8c8", fontSize: 11 }}>
        ...
        <div style={{ display: "flex", gap: 6 }}>
          {["edit", "preview", "history"].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} style={{
              ...
              {tab === "edit" ? "\u270F\uFE0F \u00C9diter" : tab === "preview" ? "\uD83D\uDC41\uFE0F Aper\u00E7u" : `\uD83D\uDCCB Historique (${savedInvoices.length})`}
            </button>
          ))}
        </div>
      </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              <Card title="Lignes de facturation">
                {invoice.items.map((item, idx) => (
                  <div key={item.id} style={{ marginBottom: 14, padding: 14, background: "#f8fafa", borderRadius: 8, border: "1px solid #e8eded" }}>
              ...
              <div style={{ display: "flex", gap: 10 }}>
                <button onClick={handleDownloadPDF} disabled={downloading} style={{ ...btnStyle, flex: 2, background: downloading ? "#88d0d0" : "#2eb8b8" }}>
                  {downloading ? "\u23F3 G\u00E9n\u00E9ration..." : "\uD83D\uDCE5 T\u00E9l\u00E9charger PDF"}
                </button>
                <button onClick={() => setActiveTab("preview")} style={{ ...btnStyle, padding: "12px 16px", background: "#eee", color: "#555" }}>\uD83D\uDC41\uFE0F</button>
                <button onClick={handleSave} disabled={saving} style={{ ...btnStyle, flex: 1, background: saving ? "#2a4a4a" : "#1a3a3a" }}>
                  {saving ? "\u23F3..." : "\uD83D\uDCBE Sauvegarder"}
                </button>
              </div>
            </div>
            ...
            {savedInvoices.length === 0 ? (
              ...
              savedInvoices.map((s) => (
                <div key={s.id} onClick={() => loadInvoice(s)} style={{
                  ...
                  padding: "14px 16px", borderRadius: 8, background: "#f8fafa", marginBottom: 8,
                  cursor: "pointer", border: "1px solid #e8eded",
                }}>
                  <div>
                    <div style={{ fontWeight: 700, fontSize: 14, color: "#333" }}>{s.number}</div>
                    <div style={{ fontSize: 12, color: "#888" }}>{s.client_name || "Client non renseign\u00E9"}</div>
                  </div>
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    ...
                    <button onClick={(e) => handleDelete(s.id, e)} style={{ background: "none", border: "none", color: "#c55", cursor: "pointer", fontSize: 16 }} title="Supprimer">\uD83D\uDDD1\uFE0F</button>
                  </div>
                </div>
              ))
            )}
...
const inputStyle = {
  ...
  fontSize: 13, color: "#333", outline: "none", background: "#fafcfc",
};
  ...
  return (
    <div ref={ref} style={{ position: "relative" }}>
      ...
      <input type="text" value={value} onChange={(e) => handleChange(e.target.value)} placeholder={placeholder}
        style={inputStyle}
        onFocus={(e) => { e.target.style.borderColor = "#2eb8b8"; if (value && filtered.length) setOpen(true); }}
        onBlur={(e) => { e.target.style.borderColor = "#dde5e5"; }}
      />
      {open && filtered.length > 0 && (
        <div style={{
          position: "absolute", top: "100%", left: 0, right: 0, zIndex: 50,
          background: "white", border: "1px solid #dde5e5", borderRadius: 6, marginTop: 2,
          ...
          {filtered.map((s, i) => (
            <div key={i} onMouseDown={() => handleSelect(s)} style={{
              ...
              background: "white", borderBottom: i < filtered.length - 1 ? "1px solid #f0f0f0" : "none",
            }}
              onMouseEnter={(e) => (e.target.style.background = "#e8f4f4")}
